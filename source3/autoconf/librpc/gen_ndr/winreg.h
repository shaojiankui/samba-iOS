...
struct winreg_StringBuf {
	...
	uint16_t size;
	const char *name;/* [size_is(size/2),length_is(length/2),charset(UTF16),unique] */
};

struct winreg_ValNameBuf {
	uint16_t length;/* [value(strlen_m_term(name)*2)] */
	uint16_t size;
	const char *name;/* [unique,charset(UTF16),length_is(length/2),size_is(size/2)] */
};
...
struct winreg_EnumValue {
	struct {
		...
		uint8_t *value;/* [range(0,0x4000000),length_is(length?*length:0),size_is(size?*size:0),unique] */
...
struct winreg_QueryValue {
	struct {
		...
		enum winreg_Type *type;/* [unique] */
		uint8_t *data;/* [range(0,0x4000000),unique,length_is(data_length?*data_length:0),size_is(data_size?*data_size:0)] */
...
struct winreg_QueryMultipleValues2 {
	struct {
		struct policy_handle *key_handle;/* [ref] */
		struct QueryMultipleValue *values_in;/* [length_is(num_values),size_is(num_values),ref] */
		uint32_t num_values;
		uint32_t *offered;/* [ref] */
		uint8_t *buffer;/* [size_is(*offered),length_is(*offered),unique] */
	} in;

	struct {
		struct QueryMultipleValue *values_out;/* [length_is(num_values),size_is(num_values),ref] */
		uint32_t *needed;/* [ref] */
		uint8_t *buffer;/* [size_is(*offered),length_is(*offered),unique] */
		WERROR result;
	} out;
