...
struct netr_ChallengeResponse {
	...
	uint8_t *data;/* [unique,size_is(length),length_is(length)] */
...
struct netr_USER_PRIVATE_INFO {
	...
	uint32_t DataLength;
	uint8_t *SensitiveData;/* [size_is(DataLength),unique,flag(LIBNDR_PRINT_ARRAY_HEX)] */
};
...
union netr_CONTROL_DATA_INFORMATION {
	...
	const char *user;/* [charset(UTF16),unique,case(NETLOGON_CONTROL_FIND_USER)] */
...
struct netr_OsVersionInfoEx {
	...
	uint32_t PlatformId;
	const char * CSDVersion;/* [flag(LIBNDR_FLAG_STR_NULLTERM|LIBNDR_FLAG_ALIGN2),subcontext_size(256),subcontext(0)] */
